...
def calc_lot_size(
    ...
    _symbol = hist_db_key.partition('_')[0]
    _timeframe = hist_db_key.partition('_')[2]
    sl_multiplier = 1.5


    def bid_ask_price(dwx, _symbol):

        # confirm if DWXClient class in use depending on attribute availability
        if hasattr(dwx, 'subscribe_symbols'):

            #Get bid & ask prices of the _symbol
            _symbol_bid, _symbol_ask = dwx.subscribe_symbols([_symbol])

            time.sleep(1)

            _symbol_bid, _symbol_ask = [dwx.market_data[_symbol].get(key) for key in ['bid', 'ask']]
            ...
            #Get bid & ask prices of the _symbol
            _symbol_bid, _symbol_ask = self.bid_ask_price(self._symbol)

            self.zmq._DWX_MTX_GET_INSTANT_RATES_(_symbol)
            time.sleep(0.05)
    ...
    def calc_sl_tp(trade_dict, bid, ask, atr):
        """[summary]
        ...
        def buy_order():
            sl_pips = ask - SL_MULTIPLIER * atr
            tp_pips = ask + TP_MULTIPLIER * atr

            return sl_pips, tp_pips

        def sell_order():
            sl_pips = bid + SL_MULTIPLIER * atr
            tp_pips = bid - TP_MULTIPLIER * atr

            return sl_pips, tp_pips

        def buy_limit_order():
            sl_pips = trade_dict['buy_sell_limit'] - SL_MULTIPLIER * atr
            tp_pips = trade_dict['buy_sell_limit'] + TP_MULTIPLIER * atr 
        ...
        sl_pips, tp_pips = order_type.get(trade_dict['_order'])()
    ...
    if new_trade_dict['instr_type'] == 'curr_mtl':
        try:

            _symbol_bid, _symbol_ask = bid_ask_price(dwx, _symbol)
            ...
            if 'USD' in _symbol:
                ...
                pip_value = 1 if 'USD' in _symbol[3:] \
                        else 1 / ((_symbol_bid + _symbol_ask) / 2)
                ...
                #else secondary currency does not form a pair with the USD as the secondary currency
                # ie. USDCAD, USDJPY...
                sec_symbol = _symbol[3:] + 'USD' if _symbol[3:] \
                        in MAJOR_CURRENCY else 'USD' + _symbol[3:]

                # Collect bid/ask prices from Market_DB
                sec_symbol_bid, sec_symbol_ask = bid_ask_price(dwx, sec_symbol)
                ...
                #ELSE USD is the second term in the currency pair, ie. NZDUSD, AUDUSD
                # Pip value is equal to 1
                pip_value = (sec_symbol_bid + sec_symbol_ask) / 2 if 'USD' in sec_symbol[3:] \
                        else 1 / ((sec_symbol_bid + sec_symbol_ask) / 2)


            #ATR value from dataframe
            atr = new_trade_df['atr'].iloc[-1]

            # Calculate risk amount of the account balance
            risk_amount = account_info['account_balance'] * risk_ratio


            # Calculate the ATR in pip value for the instrument
            #Different instruments have different relations between ATR &
            # Pip value
            atr_in_pips = atr if _symbol[:3] in \
                            ['XPT'] \
                        else atr * 10 if _symbol[:3] in \
                            ['XAU', 'XPD'] \
                        else atr * 100 if _symbol[:3] in \
                            ['XAG'] \
                                or _symbol[3:] in \
                                    ['JPY'] \
                        else atr * 1000 if _symbol[:3] in \
                            ['SEK',] \
                        else atr * 10000 if _symbol[:3] in \
            ...
            # Risk Stop Loss = stoplossMultiplier * ATR_in_pips
            calc_pip_value = risk_amount / (atr_in_pips * sl_multiplier)


            #To get the lot size, divide the current pip value of the
            # _symbol by the calculated pip value of the new trade
            # lot_size =  0.1 * calc_pip_value / pip_value
            # 0.1 refers to the lot size for a pip_value
            # ie. a pip_value is equivalent to a 0.1 lot size,
            # so what lot_size does the calc_pip_value represent?
            lot_size = 0.1 * calc_pip_value / pip_value * 0.01 if \
                            _symbol[3:] in ['JPY'] \
                        else 0.1 * calc_pip_value / pip_value if \
                            _symbol[:3] in \
                            ['XAU', 'XAG', 'XPD', 'XPT', \
                            'AUD', 'CAD', 'CHF', 'EUR', 'GBP', 'NOK', 'NZD', 'SGD', 'USD'] \
                        else None
            ...
            stop_loss, take_profit = calc_sl_tp(atr)
    ...
    # Calculations for Commodities & Indices
        ...
        try:
            # get bid ask prices
            bid, ask = self.bid_ask_price(self._symbol)
            ...
            if _symbol in ['CN50', 'US30', 'US500', 'US2000', 'NAS100', 'USDX', 'SpotBrent']:
                pip_value = 1
                ...
                indcs_sec_symbol = {
                    'AUS200': 'AUDUSD',
                    'CHINAH': 'USDHKD',
                    'GERTEC30 GER40 FRA40 SPA35 NETH25 ': 'EURUSD',
                    'UK100': 'GBPUSD',
                    'SCI25': 'USDSGD'
                }

                if hasattr(obj, name):
                    sec_symbol_bid, sec_symbol_ask = 

                else:
                    sec_symbol_bid, sec_symbol_ask = 

                if 'USD' in sec_symbol[:3]:
                    pip_value = 1 / ((sec_symbol_bid + sec_symbol_ask) / 2)
                
                else:
                    pip_value = (sec_symbol_bid + sec_symbol_ask) / 2
            ...
            atr_in_pips = atr if _symbol in \
                                ['AUS200', 'CHINAH', 'CN50','FRA40', \
                                'HK50', 'JPN225', 'GER40', 'NAS100', \
                                'SCI25', 'SPA35', 'UK100', 'US30', \
                                'US500', 'US2000','Soybeans'] \
                            else atr * 10 if _symbol in \
                                [ 'GERTEC30', 'NETH25'] \
                            else atr * 100 if _symbol in \
                                ['USDX', 'SpotCrude', 'Cotton', 'SpotBrent'] \
                            else atr * 1000 if _symbol in \
                                ['Copper'] \
                            else atr * 10000 if _symbol in \
                                ['Cattle'] \
                            else None
            ...
            # self.calc_pip_value = 50 / (self.atr_in_pips * SL_MULTIPLIER)
...
class RiskManagement():
    ...
    def __init__(self, zmq_dwx = None,
        ...
        trade_dict = new_trade_dict    #Dict containing the new trade's details
    ...
    def calc_lot(self):
        ...
        # Calculations for metal & FX pairs
        if trade_dict['instr_type'] == 'curr_mtl':
            try:
                ...
                self.calc_pip_value = self.risk_amount / (self.atr_in_pips * SL_MULTIPLIER)
